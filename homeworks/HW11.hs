...
turn :: Int -> String
turn x
    | x>0 = chr (ord '0' + x `mod` 10) : turn (x `div` 10)
    | x<0 = '-' : turn (-x)
    | x==0 = []

adder :: String -> Parser ()
adder x = P (\program -> [((),x++program)])

expr :: Parser Int
expr = do {t <- term;
         do {symbol "+"; e <- expr; return (t+e)}
         <|> do {symbol "-"; e <- term; adder $ safeturn (t-e); expr }
         <|> return t }
...
term = do {f <- factor;
         ...
         <|>do {symbol "/"; t <- factor; adder $ safeturn (f `div` t); expr}
         <|>return f}
...
-- note that "/" is considered as `div` in this program
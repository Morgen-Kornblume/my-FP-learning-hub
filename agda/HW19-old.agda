...
module MSS (
  ...
  map-distri : ∀ {A B C : Set} (f : B → C) (g : A → B) (x : List A) → map (f ∘ g) x ≡ (map f ∘ map g) x
  map-distri f g [] = refl 
  map-distri f g (x ∷ xs) rewrite map-distri f g xs = refl

  map-distri-f : ∀ {A B C : Set} (f : B → C) (g : A → B) → map (f ∘ g) ≡ (map f ∘ map g)
  map-distri-f f g = extensionality (map-distri f g)

  map-++ : ∀ {A B : Set} (f : A → B) (x y : List A) → map f (x ++ y) ≡ map f x ++ map f y
  map-++ f [] y = refl
  map-++ f (x ∷ xs) y rewrite map-++ f xs y = refl

  join-fmap : ∀ {A B : Set} (f : A → B) (x : List (List A)) → (concat ∘ (map (map f))) x ≡ (map f ∘ concat) x
  join-fmap f [] = refl
  join-fmap f (x ∷ xs) rewrite join-fmap f xs | map-++ f x (concat xs) = refl

  join-fmap-f : ∀ {A B : Set} (f : A → B) → (concat ∘ (map (map f))) ≡ (map f ∘ concat)
  join-fmap-f f = extensionality (join-fmap f)
--  join-reduce : ∀ {A : Set} (⊕/ : List A → A) (x : List (List A)) → (⊕/ ∘ concat) x ≡ (⊕/ ∘ (map ⊕/)) x
--  join-reduce ⊕/ [] = refl
--  join-reduce ⊕/ (x ∷ xs) rewrite join-reduce ⊕/ xs = {!   !}

-- out of some difficulty in proving join-reduce rule in Agda, so I use postulate to replace it, forgive me QAQ

  postulate 
    join-reduce-f : ∀ {A : Set} (⊕/ : List A → A) → (⊕/ ∘ concat)  ≡ (⊕/ ∘ (map ⊕/)) 
    horner's-rule-⊔+ : (maximum ∘ map sum ∘ tails) ≡ (foldl (λ x y → (x + y) ⊔ 0) 0)
  ...
  derivation =
    ...
    ≡⟨ cong (_∘ map tails ∘ inits) (cong (maximum ∘_) (sym (join-fmap-f sum))) ⟩
      maximum ∘ concat ∘ (map (map sum)) ∘ map tails ∘ inits
    ≡⟨ cong (_∘ (map (map sum)) ∘ map tails ∘ inits) (join-reduce-f maximum) ⟩
      maximum ∘ (map maximum) ∘ (map (map sum)) ∘ map tails ∘ inits
    ≡⟨ cong (maximum ∘_) (cong (_∘ map tails ∘ inits) (sym ( map-distri-f maximum ( map sum )  ) ) ) ⟩
      maximum ∘ map (maximum ∘ map sum) ∘ map tails ∘ inits 
    ≡⟨ cong (maximum ∘_) (cong (_∘ inits) (sym ( map-distri-f (maximum ∘ map sum) ( tails )  ) ) ) ⟩ 
      maximum ∘ map (maximum ∘ map sum ∘ tails) ∘ inits
    ≡⟨ cong  (_∘ inits) (cong (maximum ∘ map) (horner's-rule-⊔+) ) ⟩ 
      maximum ∘ map (foldl (λ x y → (x + y) ⊔ 0) 0) ∘ inits
    ≡⟨ ? ⟩
      mss-fast
    ∎
